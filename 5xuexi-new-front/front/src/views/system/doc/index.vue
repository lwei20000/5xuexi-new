...
<template>
  <div class="ele-body" v-loading="loading">
    ...
    <ele-split-layout style="min-height: calc(100vh - 117px);" class="ele-bg-white"
      width="316px"
      allow-collapse
      :leftStyle="{'border-right':'1px solid #ededed'}"
      :right-style="{ overflow: 'hidden','padding': '20px 20px 75px 20px','word-wrap': 'break-word'}"
...
import {listDocs} from "@/api/system/doc";
  ...
  methods: {
    ...
    loadData() {
      if(this.activeIndex && this.defaultRoleId){
        this.loading = true;
        listDocs({docType: this.activeIndex,defaultRoleId:this.defaultRoleId}).then((list) => {
          this.loading = false;
