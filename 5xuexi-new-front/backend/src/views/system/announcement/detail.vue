...
<template>
  <div class="ele-body "  v-loading="loading" >
    <el-card shadow="never" style="min-height: calc(100vh - 126px);" :key="announcementId">
      <div v-if="isShow">
...
<script>
import {_DetailAnnouncement} from "@/api/system/announcement";
...
export default {
  ...
  methods: {
    ...
    getDetailAnnouncement(){
      this.loading = true;
      _DetailAnnouncement(this.announcementId).then((data) => {
        this.loading = false;
        if(data){
          this.isShow = true;
          ...
          if(data.announcementAttachment){
            this.attachment = JSON.parse(data.announcementAttachment);
          }else{
            this.attachment = [];
          }
          //重新拉取未读数量
  ...
  watch: {
    $route: {
      handler(route) {
        if (route.path === '/announcement/detail') {
