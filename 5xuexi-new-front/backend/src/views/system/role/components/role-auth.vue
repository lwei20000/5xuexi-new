...
<template>
  ...
  >
    ...
    <template v-slot:footer>
      ...
      <el-button type="primary" :loading="loading" v-if="$hasPermission('sys:role:update:ea93c40c0e32419c9e3b5201f357b988')" @click="save">
        保存
      </el-button>
    </template>
  </ele-modal>
  ...
  export default {
    ...
    methods: {
      ...
      query() {
        ...
        listRoleMenus(this.data.roleId,this.appType)
          .then((data) => {
            ...
            this.$util.eachTreeData(this.authData, (d) => {
              ...
              if(this.data.systemDefault !==0 && d.systemDefault !== 0 ){
                d.disabled =  true;
              }
            });
      ...
      save() {
        ...
        updateRoleMenus(this.data.roleId,this.appType, ids)
          .then((msg) => {
            this.loading = false;
