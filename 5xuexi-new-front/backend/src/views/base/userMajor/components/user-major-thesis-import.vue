...
<script>
import { thesisImport, thesisTemplateExport} from '@/api/base/userMajor';
import {download, downloadUrl} from "@/api/common";
  ...
  export default {
    ...
    methods: {
      doDownload(){
        this.loading = true;
        thesisTemplateExport().then((data) => {
          this.loading = false;
          download(data,"用户论文成绩导入模板.xlsx");
        }).catch((e) => {
            this.loading = false;
      ...
      doUpload(file) {
        ...
        thesisImport(file)
          .then((res) => {
            ...
            if(res.code ==999){
              downloadUrl(res.data,"用户论文成绩导入_错误数据.xlsx");
            }
