...
export async function updateUserFile(data) {
  const res = await request.post('/system/user-file/update', data);
  if (res.data.code === 0) {
    return res.data.message;
  }
  return Promise.reject(new Error(res.data.message));
...
export async function removeUserFile(id) {
  const res = await request.post('/system/user-file/' + id);
  if (res.data.code === 0) {
    return res.data.message;
  }
  return Promise.reject(new Error(res.data.message));
...
export async function removeUserFiles(data) {
  const res = await request.post('/system/user-file/batch',  data );
  if (res.data.code === 0) {
    return res.data.message;
  }
  return Promise.reject(new Error(res.data.message));
